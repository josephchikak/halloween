...
//   vec3 pos = texture2D(uPositions, reference).xyz;
  ...
  vec3 noisePos = position +  amplitude * curl(frequency*position.x, frequency*position.y, frequency*position.z *0.2);
    // vElevation = elevation;

   vec3  displaced = mix(position, noisePos, 1.);
...
//   float elevation = sin(modelPosition.x * 0.2) * sin(modelPosition.z *0.5) * 0.4;
    //  elevation -= abs(cnoise(vec3(modelPosition.xz * 3.0, 1.5)) * 0.2);
